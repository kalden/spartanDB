...
#'
#' @param dblink A link to the database in which this table is being created
#' @param ensemble Ensemble object created by spartan
#' @param parameters The parameters of the simulation that are being analysed
#' @param measures The measures of the simulation that are being assessed
#' @param data_to_predict Sets of parameters for which simulation output measures should be predicted
#' @param normalise Whether the data_to_predict should be normalised prior to making predictions. Defaults to FALSE
...
#' @param experiment_date Date this experiment was performed. Defaults to that day's date
#' @export
use_ensemble_to_make_and_store_predictions<-function(dblink, ensemble, parameters, measures, data_to_predict, normalise=FALSE, normalise_result=FALSE, experiment_description,
                                                      experiment_date=Sys.Date())
{
  tryCatch({
    ...
    if(experiment_id != -1)
    {
      ...
      # Generate predictions
      predictions <- spartan::use_ensemble_to_generate_predictions(ensemble, data_to_predict, parameters, measures,
                                                          normalise_values = normalise, normalise_result = normalise_result)
      ...
      emulator_predictions<-cbind(predictions, rep("Ensemble",nrow(predictions)),
                                  param_ids, rep(experiment_id,nrow(predictions)))
  ...
  {
    message(paste("Error in Using Ensemble to Generate Predictions and Add that Experiment to the Database.Error Message Generated: \n",e,sep=""))
    remove_errored_new_experiment_from_db(dblink, experiment_id)
  })
}
