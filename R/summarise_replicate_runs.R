    ...
    if(experiment_id != -1)
    {
      ...
      if(pre_existing_results==0)
      {
        ...
        if(num_results > 0)
        {
          ...
          out<-RMySQL::dbWriteTable(dblink, value = as.data.frame(results_summary),row.names=FALSE,name="spartan_analysed_results", append=TRUE)

          message(paste0(num_results," summarised and added to database"))
        }
        else
        {
          message("Error in summarising replicate experiments. No results in the DB for this experiment.")
...
summarise_replicate_efast_runs<-function(dblink, parameters, measures, experiment_id=NULL, experiment_description=NULL, experiment_date=NULL)
{
  ...
  tryCatch({
    ...
    if(experiment_id != -1)
    {
      # Check that there are no results already in the DB
